...
class Table {
  constructor(table, parentInstance) {
    ...
    this.rowsPerPage = parseInt(
      table.getAttribute(this.parentInstance.rowsPerPageAttribute) ?? this.parentInstance.rowsPerPage
    );
  ...
  renderPagination() {
    ...
    const createPageLink = (page, isActive = false, isDisabled = false, text = null) => {
    ...
    if (totalPages <= 7) {
      ...
      for (let i = 1; i <= totalPages; i++) {
        ul.appendChild(createPageLink(i, this.currentPage === i));
      }
    } else {
      // If more than 7 pages, use ellipsis logic
      // First page
      ul.appendChild(createPageLink(1, this.currentPage === 1));
      ...
      // Last page
      ul.appendChild(createPageLink(totalPages, this.currentPage === totalPages));
    }
      ...
      this.deleteSelectedSelector.addEventListener("click", (e) => {
        ...
        if (selectedRows.length > 0) {
          const { modal, confirmButton } = this.alert(
            selectedRows.length > 1 ? this.deleteMultipleRowsMessage : this.deleteRowMessage
          );
